...
class Heap(Generic[T]):
    ...
    def reheap_up(self, index):
        ...
        if self.compare(self.heapArr[index], self.heapArr[parent_index]) < 0:
            return
        else:
    ...
    def reheap_down(self, index):
        ...
        if (left_index >= self.count): return

        if (right_index >= self.count):
            if self.compare(self.heapArr[index], self.heapArr[left_index]) < 0:
                self.swap(self.heapArr, index, left_index)
            return

        if self.compare(self.heapArr[left_index], self.heapArr[right_index]) > 0:
            larger_index = left_index
        else:
            larger_index = right_index

        if self.compare(self.heapArr[index], self.heapArr[larger_index]) > 0:
            return
        else:
